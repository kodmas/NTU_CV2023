...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            points = np.round(corners[0][0]).astype(int)
            ...
            frame = warping(ref_image, frame, H,np.min(points,axis=0)[1], np.max(points,axis=0)[1], np.min(points,axis=0)[0], np.max(points,axis=0)[0], direction='b')
